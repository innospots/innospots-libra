...
package io.innospots.workflow.console.converter.apps;
 ...
 */
@Mapper
public interface AppNodeGroupConverter extends BaseBeanConverter<AppNodeGroup,AppNodeGroupEntity> {

    AppNodeGroupConverter INSTANCE = Mappers.getMapper(AppNodeGroupConverter.class);
    ...
    default AppNodeGroupBaseInfo modelToBase(AppNodeGroup nodeGroup) {
        ...
        if (CollectionUtils.isNotEmpty(nodeGroup.getNodes())) {
            baseInfo.setNodes(AppNodeDefinitionConverter.INSTANCE.modelToInfoList(nodeGroup.getNodes()));
        }
        return baseInfo;
    }
