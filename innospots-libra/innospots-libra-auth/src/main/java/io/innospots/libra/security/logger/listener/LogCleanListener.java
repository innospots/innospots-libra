...
package io.innospots.libra.security.logger.listener;
 ...
 * @date 2023/11/10
 */
public class LogCleanListener implements IEventListener<LogClearEvent> {

    private final LoginLogOperator loginLogOperator;

    public LogCleanListener(LoginLogOperator loginLogOperator) {
        this.loginLogOperator = loginLogOperator;
    }

    @Override
    public Object listen(LogClearEvent event) {
        if(Objects.equals(event.getLogType(),"login")){
            loginLogOperator.deleteLogHis(event.getMaxCount(),event.getKeepDays());
        }
        return null;
    }
}
